...
function snapshot(language: string): IdentityBaselineSnapshot {
  return {
...
describe('identity self-healing end-to-end', () => {
  it('flows baseline -> drift -> health -> diagnosis -> repair -> verify -> baseline v2 -> healthy', async () => {
    ...
    expect(intelligence.diagnosis.issues.map((issue) => issue.key))
      .toContain('identity-drift:locale.language')
    expect(intelligence.diagnosis.issues[0]?.repairPolicy).toBe('confirm_apply')
    ...
    expect(plan.diagnosedIssueKeys).toContain('identity-drift:locale.language')
